-- Fix register_profile: keep role in sync when the same auth user re-registers
-- (e.g. influencer signup after an earlier business profile row was created).
...
-- Optional: fix accounts that already have influencer data but wrong role
